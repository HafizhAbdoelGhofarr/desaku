    ...
    <div className="flex flex-col min-h-screen">
      ...
      <header className="px-6 lg:px-14 h-20 flex items-center justify-between border-b bg-white/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <nav className="flex items-center gap-4">
          <Link href="/publik" className="text-sm font-medium text-slate-600 hover:text-emerald-700 transition-colors">
            Portal Publik
          </Link>
          <Link
            href="/login"
            className="text-sm font-medium bg-slate-900 text-white px-5 py-2.5 rounded-full hover:bg-slate-800 transition-all shadow-md hover:shadow-lg"
          >
            Masuk Sistem
          </Link>
        </nav>
      ...
      <main className="flex-1 flex flex-col items-center text-center px-4 pt-24 pb-16 bg-gradient-to-b from-emerald-50 to-white overflow-hidden relative">
        ...
        <p className="mt-8 text-lg md:text-xl text-slate-600 max-w-2xl leading-relaxed">
          Platform terpadu untuk Dinas Pemberdayaan Masyarakat dan Desa (DPMD) beserta pengelola desa dalam mengukur, memverifikasi, dan mensimulasikan indeks ketahanan desa secara cerdas.
        </p>

        <div className="mt-10 flex flex-col sm:flex-row gap-4">
          <Link
            href="/login"
            className="flex items-center gap-2 bg-emerald-600 text-white px-8 py-4 rounded-full text-lg font-semibold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20 hover:-translate-y-1"
          >
            Mulai Sekarang
          ...
          <Link
            href="/publik"
            className="flex items-center gap-2 bg-white text-slate-700 border border-slate-200 px-8 py-4 rounded-full text-lg font-medium hover:bg-slate-50 transition-all"
          >
            Lihat Skor Desa
          </Link>
        </div>
        ...
        <div className="mt-32 grid md:grid-cols-3 gap-8 max-w-6xl w-full text-left">
          <FeatureCard
            ...
            desc="Data ketahanan desa diverifikasi oleh admin DPMD sebelum dipublikasikan, menjamin kredibilitas."
          />
          <FeatureCard
            icon={<BarChart3 className="w-8 h-8 text-blue-600" />}
            title="What-If Analysis"
            desc="Simulasikan dampak kebijakan secara real-time dengan bantuan AI untuk pengambilan keputusan strategis."
          />
          <FeatureCard
            icon={<Users className="w-8 h-8 text-orange-600" />}
            title="Transparansi Publik"
            desc="Masyarakat dapat memantau secara langsung tingkat ketahanan dan kemajuan desanya."
          />
        </div>
      </main>
